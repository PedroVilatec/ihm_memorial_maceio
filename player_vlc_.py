		...
		if self.longPress == False:
			...
			if self.playlistView.currentIndex().isValid():            
				if not any(nameMedia[-4:] in item for item in self.config["VIDEO_FILES"]):
					if self.playlistView.currentIndex().isValid():
						print("get chapter", self.mediaplayer.get_chapter())
						self.playerState = QMediaPlayer.StoppedState    
						self.playlist.setCurrentIndex(self.playlistView.currentIndex().row())
						print("current index_row", self.playlistView.currentIndex().row())
						print("MEDIA INSTANCE", self.listPlayer.get_media_player().get_media())
						print("URL", self.listPlayer.get_media_player().get_media().get_mrl())
						print("CHAPTER", self.listPlayer.get_media_player().get_chapter())
