...
#######################################################################
		...
		for loop in range(self.mediaList.count()):
			#rotina para alterar o botão play/pause
			self.vlc_media_event_manager = self.mediaList.__getitem__(loop).event_manager()
			self.vlc_media_event_manager.event_attach(vlc.EventType.MediaStateChanged, class_controls.setState)		
...
##                    self.playlist.load(url)
	...
	def mouseReleaseEvento(self, event):
		...
		'''
		try:
			print("released", self.playlistView.currentIndex().isValid())
