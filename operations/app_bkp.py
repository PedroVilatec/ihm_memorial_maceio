...
import json
import threading
import player_vlc_1_0 as player_
...
class Ui(QtWidgets.QMainWindow):
	def __init__(self, xml):
		...
		# ~ print(os.path.join(ROOT_DIR,"IHM")) 
	...
	def appSignalLongPress(self, dict_media):
		if self.config is not None and self.senhaEnabled:
			if any(dict_media["name"][-4:] in item for item in self.config["VIDEO_FILES"]):
				...
				url = ROOT_DIR +"/ihm/media_shared/video/"+dict_media["name"]
				file_exist = False
				if a == 0 and self.config is not None:
					...
					# self.config['VIDEO_EXTERNO']['URL'] = os.path.join(ROOT_DIR, "cerimonia", "videos", nameMedia)
					# self.config['VIDEO_IHM'] = "VIDEO_EXTERNO"
					# self.sendConfig()   
			...
			if file_exist: 
				a = self.msgBoxFunction("O arquivo {} já existe no destino. Deseja exibi-lo na cerimônia? ","Ok", "Cancelar", mqtt=True, timeout = 30)
				if a == 0:
					pass
...
class SmbThread(QtCore.QThread):
	...
	def run(self):
		while True:
			print('sudo mount -t cifs //ECUMENICA.local/media ' + os.path.join(ROOT_DIR, "ihm/media_shared", "") + ' -o username=pi,password=nbr5410!')
			p = subprocess.Popen('sudo mount -t cifs //ECUMENICA.local/media ' + os.path.join(ROOT_DIR, "ihm/media_shared", "") + ' -o username=pi,password=nbr5410!', shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
						close_fds=False)
